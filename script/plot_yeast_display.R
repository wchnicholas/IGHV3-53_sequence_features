...
enrich_table <- read_csv("result/CDR3_LIB_enrichment.csv") %>%
                  select(-X1) %>%
                  mutate(bind=bind_average_enrich) %>%
                  mutate(exp=exp_average_enrich) %>%
...
p_KV19_bind_cor  <- Plot_RepvsRep(ggplot(enrich_table,aes(x=bind_rep1_enrich,y=bind_rep2_enrich)),
                                  expression(bold(log['10']~enrichment~'(replicate 1)')), 
                                  expression(bold(log['10']~enrichment~'(replicate 2)')),
                                  'Binding')
p_KV19_exp_cor   <- Plot_RepvsRep(ggplot(enrich_table,aes(x=exp_rep1_enrich,y=exp_rep2_enrich)),
                                  expression(bold(log['10']~enrichment~'(replicate 1)')), 
                                  expression(bold(log['10']~enrichment~'(replicate 2)')),
                                  'Expression')
print ("Pearson correlation between binding replicates:")
print (cor(enrich_table$bind_rep1_enrich,enrich_table$bind_rep2_enrich))
print ("Pearson correlation between expression replicates:")
print (cor(enrich_table$exp_rep1_enrich,enrich_table$exp_rep2_enrich))
